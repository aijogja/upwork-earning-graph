...
class AuthViewTestCase(TestCase):
    ...
    def test_auth_view_redirects_to_authorization_url(self, mock_get_client):
        ...
        mock_client.get_authorization_url.return_value = ('https://upwork.com/oauth', 'test_state')
    ...
    def test_callback_with_state_mismatch(self, mock_get_client):
        ...
        response = self.client.get(reverse('callback'), {'code': 'test_code', 'state': 'wrong_state'})
        self.assertEqual(response.status_code, 400)
    ...
    def test_callback_success_flow(self, mock_authenticate, mock_get_client, mock_graphql_api, mock_login):
        session = self.client.session
            ...
            'data': {
                'user': {
                    ...
                    'freelancerProfile': {
                        ...
                        'lastName': 'User',
                        'personalData': {
                            'profileUrl': 'https://upwork.com/profile'
                        }
                    }
                }
            }
        ...
        test_user = User.objects.create_user(username='test_rid', email='test@example.com')
        mock_authenticate.return_value = test_user

        response = self.client.get(reverse('callback'), {'code': 'test_code', 'state': 'test_state'})
